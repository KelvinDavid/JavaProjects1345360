...
#Question 09 - IDLE program
#==========================================
#This program is the modificaiton of 
#question 8 kernel so when all programs
#exit the scheduler will store a 
#state in an infinite loop called IDLE state
...
main:
	...
	la	$1, IDLE_pcb		#load IDLE task
	
	la	$2, IDLE_pcb		#link IDLE to itself
	sw	$2, pcb_link($1)	

	la	$2, IDLE_stack		#set $sp to top of stack
	sw	$2, pcb_sp($1)

	la	$2, IDLE_main		#set IDLE main
	sw	$2, pcb_ear($1)		#into ear
...
removeTask:
	...
	sequ	$3, $1, $2		#if removing last task
	bnez	$3, IDLEJ		#set to IDLE
...
IDLE_main:
	...
	addui	$1, $0, 0x40	
	sw	$1, SSD_LR($0)
	addui	$1, $0, 0	#loop print '-. -' sleepy face
	sw	$1, SSD_LL($0)
	addui	$1, $0, 0x80		
	sw	$1, SSD_UR($0)
	addui	$1, $0, 0x40
